...
from src.utils.plots import plot_pr, plot_reliability, plot_roc, plot_attention_bar, plot_topk_slices_grid
...
def run_federated(
    ...
    for round_idx in range(cfg.rounds):
        ...
        for client in clients:
            ...
            if cfg.log_client_metrics:
                log_metrics({f"client_{client.client_id}_loss": metrics["train_loss"]}, step=round_idx)
            ...
            if private_state:
                torch.save(private_state, ckpt_dir / f"private_{client.client_id}_round{round_idx}.pt")
    ...
    keys = [k for k in history[-1].keys() if k not in {"round"}]
...
def evaluate_holdouts(
    ...
    sens = _sensitivity_at_specificity(np.array(y_true_all), np.array(y_prob_all), specificity_target)
    ...
    artifacts.append(plot_roc(np.array(y_true_all), np.array(y_prob_all), output_dir / "roc.png"))
    artifacts.append(plot_pr(np.array(y_true_all), np.array(y_prob_all), output_dir / "pr.png"))
...
def _sensitivity_at_specificity(y_true: np.ndarray, y_score: np.ndarray, spec: float) -> float:
    fpr, tpr = _roc_curve(y_true, y_score)
    specificity = 1 - fpr
...
def _confusion_matrix(y_true: np.ndarray, y_prob: np.ndarray, thresh: float = 0.5) -> Dict[str, int]:
