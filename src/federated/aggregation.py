...
class FedAdam(Aggregator):
    ...
    def __init__(self, lr: float = 1.0, beta1: float = 0.9, beta2: float = 0.999, tau: float = 1e-3) -> None:
        if lr <= 0:
    ...
    def aggregate(
        ...
        for key in global_state.keys():
            delta = None
            for state, w in zip(client_states, weights):
                diff = (state[key].detach().float() - global_state[key].detach().float()) * (w / total)
            ...
            new_state[key] = (global_state[key].detach().float() + self.lr * update).to(global_state[key].dtype)
        return new_state
