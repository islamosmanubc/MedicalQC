...
def test_uniform_sampling_deterministic(tmp_path: Path):
    ...
    ds1 = StudyFolderDataset(root=tmp_path, sampling=SamplingConfig(max_slices=4, strategy="uniform"), seed=123)
    ds2 = StudyFolderDataset(root=tmp_path, sampling=SamplingConfig(max_slices=4, strategy="uniform"), seed=999)
...
def test_random_sampling_deterministic(tmp_path: Path):
    _write_study(tmp_path, "hospital_000", "study_000", 10)
    ds1 = StudyFolderDataset(root=tmp_path, sampling=SamplingConfig(max_slices=4, strategy="random"), seed=123)
    ds2 = StudyFolderDataset(root=tmp_path, sampling=SamplingConfig(max_slices=4, strategy="random"), seed=123)
